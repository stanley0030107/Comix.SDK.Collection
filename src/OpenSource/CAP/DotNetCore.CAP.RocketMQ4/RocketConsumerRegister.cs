    ...
    public class RocketConsumerRegister : IConsumerRegister
     {
        ...
        public RocketConsumerRegister(ILogger<RocketConsumerRegister> logger, IServiceProvider serviceProvider)
        {
            ...
            _rocketOptions = serviceProvider.GetService<IOptions<RocketMQOptions>>().Value;
        ...
        private void RegisterMessageProcessor(IConsumerClient client)
        {
            ...
            {
                _logger.LogDebug($"Received message. id:{transportMessage.GetId()}, name: {transportMessage.GetName()}");
                ...
                try
                {
                    ...
                    try
                    {
                        if (!canFindSubscriber)
                        {
                            ...
                            if (!canFindSubscriber)
                            {
                                ...
                                {
                                    _logger.LogWarning($"Topic[{name}] 没有处理器，直接抛弃 id:{transportMessage.GetId()}");
                                    return;
                                }
                    ...
                    catch (Exception e)
                    {
                        ...
                        else
                        {
                            ...
                            message = new Message(transportMessage.Headers, transportMessage.Body);
                        }
                    }
