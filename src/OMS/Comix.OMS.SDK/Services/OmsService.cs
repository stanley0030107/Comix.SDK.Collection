...
public class OmsService : IOmsService
{
    ...
    public OmsService(ISqlSugarClient db,
        IHttpClientFactory httpClientFactory,
        ILogger<OmsService> logger)
    {
    ...
    /// </summary>
    /// <returns></returns>
    public async Task<List<CustomerBalanceSimpleDto>> GetOmsBalance()
    {
        var customerBalanceRepo= App.GetService<SqlSugarRepository<CustomerBalance>>();
        await customerBalanceRepo.Context.Ado.UseStoredProcedure().ExecuteCommandAsync("PrCalculate_Customer_Balance");
        var dtos = await customerBalanceRepo.AsQueryable()
            .Select(o => new CustomerBalanceSimpleDto
            {
                Amount = o.Amount,
                CustCode = o.CustCode
            }).ToListAsync();
    ...
    public async Task<string> ExecuteReturnStringAsync(string path, object req)
    {
        ...
        if (_omsOptions.Url.EndsWith("/") && path.StartsWith("/"))
        {
            path = path.TrimStart('/');
        }

        var url = $"{_omsOptions.Url}{path}";
